...
<details>
  <summary>📝<strong>Ejercicio 1</strong>: del fichero `starwars` encuentra todos los personajes que el peso esté entre 60 y 90, de ojos que no sean azules, y que tengan menos de 100 años. Haz primero que se impriman todas las columnas. Muestra luego solo las columnas `name`, `mass`, `eye_color` y `birth_year`.
...
filtro <-
  starwars %>%
  filter(between(mass, 60, 90) & eye_color != "blue" & birth_year < 100)
...
<details>
  <summary>📝<strong>Ejercicio 2</strong>: selecciona solo las columnas que contengan variables numéricas, coloca los años de nacimiento como primera columna y cambia los nombres a castellano.
...
<details>
  <summary>📝<strong>Ejercicio 3</strong>: calcula una nueva columna que indique el número de naves que ha pilotado cada persona (escribe `? starwars` en consola para ver documentación del fichero). Filtra después solo aquellos personajes que han conducido alguna nave, y crea una nueva variable `TRUE/FALSE` que nos diga si ha conducido o no alguna nave.
...
print(starwars %>%
        mutate(n_naves = map_chr(starships, length)), width = Inf)
...
<details>
  <summary>📝<strong>Ejercicio 4</strong>: ordena a los personajes de mayor a menor número de películas en las que ha aparecido
  
<!-- toc -->
- Solución:

```{r}
starwars %>% # Calculamos el número de películas
...
<details>
  <summary>📝<strong>Ejercicio 5</strong>: selecciona 9 personajes al azar. Después selecciona los 5 personajes que en más películas han salido y los 5 que menos.
...
# personas que en más películas han salido (metiendo empates)
starwars %>%
  mutate(n_films = map_int(films, length)) %>%
  slice_max(n_films, n = 5)

# personas que en más películas han salido (sin empates)
starwars %>%
  mutate(n_films = map_int(films, length)) %>%
  slice_max(n_films, n = 5, with_ties = FALSE)

# personas que en menos películas han salido (metiendo empates)
starwars %>%
  mutate(n_films = map_int(films, length)) %>%
  slice_min(n_films, n = 5)

# personas que en menos películas han salido (sin empates)
starwars %>%
  mutate(n_films = map_int(films, length)) %>%
...
<details>
  <summary>📝<strong>Ejercicio 6</strong>: selecciona las columnas que se refieren a variables de color (ojos, piel, pelo).
