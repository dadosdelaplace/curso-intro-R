...
- **Paso 1**: entra en la web <https://cran.r-project.org/> y en la pantalla de inicio **selecciona la instalación acorde a tu sistema operativo** (ver imagen \@ref(fig:pantalla-cran))

```{r pantalla-cran, echo = FALSE, out.width = "60%", fig.align = "center", fig.cap = "Pantalla inicial de la plataforma CRAN de R."}
...
```{r pantalla-mac, echo = FALSE, out.width = "60%", fig.align = "center", fig.cap = "Pantalla de instalación de R en Mac OS."}
...
```{r pantalla-windows-1, echo = FALSE, out.width = "60%", fig.align = "center", fig.cap = "Pantalla previa de instalación de R en Windows."}
...
```{r cranR, echo = FALSE, out.width = "55%", fig.align = "center", fig.cap = "Primera pantalla al abrir el ejecutable de R."}
knitr::include_graphics("./img/cranR.jpg")
```

Para comprobar que está correctamente instalado, **prueba a escribir en la consola** el siguiente código (**recuerda**: los códigos puedes copiarlos directamente de la cajita haciendo click en el botón de la esquina superior derecha)
...
```{r primera-suma, echo = FALSE, out.width = "70%", fig.align = "center", fig.cap = "Primera suma en la consola de R."}
knitr::include_graphics("./img/primera_suma.jpg")
```

**¡Enhorabuena!*

Si te ha devuelto `[1] 3`, ya has hecho más de lo que parece: has definido dos variables `a` y `b`, has **asignado un valor numérico a cada variable** y las hemos usado. **Ya sabemos usar `R` como calculadora**.

El valor `[1]` antes del resultado en realidad nos numera las líneas de la consola usadas.
...
```{r pantalla-rstudio, echo = FALSE, out.width = "60%", fig.align = "center", fig.cap = "Descargar el ejecutable de RStudio para su posterior instalación."}
...
```{r inicio-rstudio-1, echo = FALSE, out.width = "65%", fig.align = "center", fig.cap = "Primer recibimiento de nuestro mejor amigo RStudio."}
...
* **`r colorize("Consola", "#4197D2")`**: es el nombre para llamar a esa ventana grande que te ocupa la mayor parte de tu pantalla. Prueba a escribir el mismo código que antes en ella (es el equivalente a la consola de R que hemos abierto al principio).
...
Ese código no se ejecuta salvo que se lo digamos. Tenemos principalmente dos opciones para ello: o copiamos el trozo de código que queramos y lo pegamos en la consola (como hemos hecho al principio), o **`r colorize("activamos el cuadrado Source on save a la derecha del botón de guardar", "#20935E")`** y volvemos a hacer click en el botón de guardar: siempre que esa opción esté activada, al guardar no solo es nos guarda el archivo `.R` sino que además se ejecuta solo y nos devuelve los resultados por consola.
